...
// Vulnerable utility functions added for testing
export const executeCommand = (cmd: string) => {
  const { exec } = require('child_process')
  // Command injection vulnerability
  return exec(cmd, (error: any, stdout: any, stderr: any) => {
    if (error) {
...
export const unsafeEval = (userInput: string) => {
  // Code injection vulnerability
  return eval(userInput)
}

export const weakRandomNumber = () => {
...
export const vulnerableRegex = (input: string) => {
  // ReDoS vulnerability
  const regex = /^(a+)+$/
...
// Insecure comparison
export const insecureCompare = (hash1: string, hash2: string) => {
  return hash1 === hash2
}
...
var globalPassword = "admin123" // Leaked global variable

export const logSensitiveData = (data: any) => {
  // Information disclosure through logging
  console.log("User data:", JSON.stringify(data))
  console.log("Environment variables:", process.env)
}

export const downloadFileUnsafe = (url: string) => {
  // SSRF vulnerability
  const https = require('https')
  return https.get(url, (res: any) => {
